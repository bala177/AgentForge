...
def delete_activity(activity_id: str) -> bool:
    """Delete a single activity by ID. Returns True if found and deleted."""
    activities = _load_activities()
    before = len(activities)
    activities[:] = [a for a in activities if a["id"] != activity_id]
    if len(activities) < before:
        _flush_activities()
        log.info("Activity deleted: %s", activity_id)
        return True
    return False


def clear_activities() -> int:
    """Delete all activity entries. Returns count deleted."""
    global _activity_cache
    activities = _load_activities()
    count = len(activities)
    _activity_cache = []
    _flush_activities()
    log.info("All %d activities cleared", count)
    return count
