"""
llm/provider.py — LLM abstraction layer.
...
class LLMProvider:
    ...
    def chat_stream(self, messages: list[dict]):
        ...
        if provider == "ollama":
            ...
            # Keyword or unknown: yield full response at once
            try:
    ...
    def _stream_ollama(self, messages: list[dict]):
        """Stream tokens from Ollama (NDJSON per line, stream=true)."""
        url = f"{self.config.ollama_url}/api/chat"
        payload = {
            "model": self.config.model,
            "messages": messages,
        ...
        try:
            ...
            log.error("Ollama streaming error: %s — falling back to sync", e)
            try:
                yield self._chat_ollama(messages, json_mode=False)
            except Exception:
                yield ""

    def _stream_openai(self, messages: list[dict]):
        """Stream tokens from OpenAI (Server-Sent Events, stream=true)."""
        url = f"{self.config.openai_base_url}/chat/completions"
        headers = {
            "Content-Type": "application/json",
            "Authorization": f"Bearer {self.config.openai_api_key}",
        ...
        try:
            ...
            log.error("OpenAI streaming error: %s — falling back to sync", e)
            try:
                yield self._chat_openai(messages, json_mode=False)
            except Exception:
                yield ""
